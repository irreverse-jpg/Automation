...
const BASE_URL = (__ENV.BASE_URL || __ENV.WITHERS_BASE_URL || __ENV.K6_BASE_URL || '').replace(/\/+$/, '');
const SELECTED_SCENARIO = (__ENV.SCENARIO || 'all').trim().toLowerCase();
const ORIGIN = (BASE_URL.match(/^https?:\/\/[^/]+/i) || [''])[0];

if (!BASE_URL) {
    throw new Error('BASE_URL is required. Example: BASE_URL=https://w-uat.hosted.positive.co.uk/en-gb');
}
...
What this file does
-------------------
- Runs simple GET-only journeys against core Withers pages.
...
Replace BASE_URL if needed. For Withers this should include the locale path, for example:
...
--------------------------------
- Core localized pages under BASE_URL:
    - /
    - /people
    - /locations
    - /insight
...
export function handleSummary(data) {
    ...
    const failedChecks = checks.filter((checkResult) => checkResult.fails > 0);
    const hasReliabilityFailure = failedThresholds.some((item) => item.metricName === 'errors' || item.metricName === 'http_req_failed');
    const hasLatencyFailure = failedThresholds.some((item) => item.metricName === 'http_req_duration' || item.metricName === 'http_req_waiting');
    ...
    const lines = [
        ...
        ...(failedChecks.length
            ? failedChecks.map((checkResult) => `- ${checkResult.name}: fails=${checkResult.fails}, passes=${checkResult.passes}`)
            : ['- No failing checks']),
        '',
        'How to read quickly:',
...
function requestAndCheck(path, tags = {}, optionsForRequest = {}) {
    ...
    const ok = check(res, {
        [`${path} status is 2xx/3xx`]: (response) => response.status >= 200 && response.status < 400,
        [`${path} has content`]: (response) => (response.body || '').length > 0,
    });
...
export function browseCorePages() {
    ...
    requestAndCheck('/people');
    sleep(Math.random() * 1 + 0.2);

    requestAndCheck('/locations');
    sleep(Math.random() * 1 + 0.2);

    requestAndCheck('/insight');
    sleep(Math.random() * 1 + 0.2);

    requestAndCheck('/search');
    sleep(Math.random() * 1 + 0.2);
