...
test('Non-Functional - Structured data (JSON-LD) exists and is valid JSON', async ({ page }) => {
    await test.step('Open the homepage and verify JSON-LD structured data', async () => {
        ...
        if (microdataCount > 0) {
            const itemtypes = await microdataRoots.evaluateAll(nodes => nodes.map(node => node.getAttribute('itemtype') || node.getAttribute('typeof') || node.getAttribute('property') || '').filter(Boolean));
